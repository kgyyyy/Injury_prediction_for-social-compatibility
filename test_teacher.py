...
def test(model, loader):
    ...
    with torch.no_grad():
        for batch_x_acc, batch_x_att, batch_y_HIC, batch_y_AIS in loader:
            batch_x_acc, batch_x_att = batch_x_acc.to(device), batch_x_att.to(device)
...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load("./ckpt/teacher_best.pth"))

    HIC_preds, HIC_trues = test(model, test_loader)
    HIC_preds[
        HIC_preds > 2500] = 2500  # Set upper bound: HIC > 2000 generally indicates critical injury and is not meaningful
...
"""
    ...
    file_path = "./results/teacher_model_results.md"
    ...
    plt.savefig('results/teacher_model.png', dpi=300)
    plt.show()
